	...
	public class CadastrarNotaBehind : ComponentBase
	{
		...
		protected async void SaveNota(EditContext e)
		{
			try
			{
				...
				if (Nota.NotaItens.Any() is false) throw new Exception(Messages.NotInclude);
				if (string.IsNullOrWhiteSpace(nota.Description)) throw new Exception(String.Format(Messages.ExisteRegister, "Descrição"));
				...
				AlertEdicao.ShowSuccessMessage(Messages.Save);
